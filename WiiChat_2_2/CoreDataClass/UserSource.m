...
-(id)insertUserWithID1:(NSString*)ID Password:(NSString*)password{
    AllUsers *requestUser = [self checkUserInDatabaseWithID:ID Password:password];
    NSLog(@"查询数据源结果:%@",requestUser);
    NSEntityDescription *ent = [NSEntityDescription entityForName:@"User" inManagedObjectContext:self.localManagedObjectContext];
    ...
    if (requestUser.wiiError!=nil) {//若有错误信息 则直接返回错误信息
        ...
        if ([requestUser.wiiLogin isEqualToString:@"NO"])//NO-未在其他设备上登录
        {
            ...
            //            NSError *err2;
            //           NSArray<AllUsers*> *result = [self.sourceManagedObjectContext executeFetchRequest:request error:&err2];
            ...
            ////                obj.wiiLogin = @"YES";
            ////            }];
            //            if (self.sourceManagedObjectContext.hasChanges) {
            //                [self.sourceManagedObjectContext save:nil];
            //            }
            //            if (err2) {
            ...
            //            NSError *err3;
            //            NSArray<AllUsers*> *result1 = [self.sourceManagedObjectContext executeFetchRequest:request1 error:&err3];
            //            NSLog(@"102查找结果：%@",[result1 firstObject]);
            NSError *err1;
            requestUser.wiiLogin = @"YES";
            if (self.sourceManagedObjectContext.hasChanges) {
                [self.sourceManagedObjectContext save:&err1];
            }
            if (err1) {
                newUser.wiiError = Error_008;
            ...
            requestUser = [self checkUserInDatabaseWithID:ID Password:password];//修改数据源login信息后重新获取数据源信息
            if ([requestUser.wiiLogin isEqualToString:@"YES"]) {
                [self copyInfoFromAllUser:requestUser ToUser:newUser];
                newUser.wiiLogin = @"YES_Pass";//通过验证且未在其他设备上登录
                
                NSError *err2;
                if (self.localManagedObjectContext.hasChanges) {
                    [self.localManagedObjectContext save:&err2];
                }
                if(err2){
...
-(id)insertUserWithID:(NSString*)ID Password:(NSString*)password{
    User *newUser = [NSEntityDescription insertNewObjectForEntityForName:@"User" inManagedObjectContext:self.localManagedObjectContext];
    sourceResultDic = [NSMutableDictionary dictionaryWithCapacity:10];
    sourceResultDic = [self checkUserInDatabaseWithID1:ID Password:password];
    ...
    if ([sourceResultDic valueForKey:@"wiiError"]!=nil) {//若有错误信息 则直接返回错误信息
        ...
        if ([[sourceResultDic valueForKey:@"wiiLogin" ] isEqualToString:@"NO"])//NO-未在其他设备上登录
        {
            ...
            NSArray<AllUsers*> *result = [self.sourceManagedObjectContext executeFetchRequest:request error:&err1];
            [result enumerateObjectsUsingBlock:^(AllUsers * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                obj.wiiLogin = @"YES";
            }];
            if (self.sourceManagedObjectContext.hasChanges) {
            ...
            //if ([requestUser.wiiLogin isEqualToString:@"YES"]) {
            [UserSource copyInfoFromSourceUserDic:sourceResultDic toNewUser:newUser];
            newUser.wiiLogin = @"YES_Pass";//通过验证且未在其他设备上登录
            NSLog(@"插入本地数据库中内容为：%@",newUser);
            if (self.localManagedObjectContext.hasChanges) {
                [self.localManagedObjectContext save:nil];
            }
            return newUser;//存储至本地成功，返回newUser
            // }
...
-(NSString*)insertUser:(User*)user{//顶号登录
    NSString *result;
    User *newUser = user;
    [self.localManagedObjectContext insertObject:newUser];
    NSError *err;
    if (self.localManagedObjectContext.hasChanges) {
        [self.localManagedObjectContext save:&err];
    }
    if (err) {
...
-(User*)getUser{
    ...
    requset.entity = [NSEntityDescription entityForName:@"User" inManagedObjectContext:self.localManagedObjectContext];
    //[NSFetchRequest fetchRequestWithEntityName:@"User"];
    NSError *err;
    NSArray<User*> *result = [self.localManagedObjectContext executeFetchRequest:requset error:&err];
    requset = nil;
    if (err) {
...
-(NSMutableDictionary*)getUserInfo{
    ...
    [UserSource copyInfoFromUser:[self getUser] toUserInfoDic:userInfoDic];
    return userInfoDic;
}
-(NSString*)resignUser1{
    NSString *resignResult;
    NSMutableDictionary *tempDic = [NSMutableDictionary dictionaryWithCapacity:1];
    User *tempUser = [self getUser];
    if ([tempUser.wiiID isEqualToString:@""]) {
        ...
        if (sourceUsers.wiiError!=nil) {//确定数据源中存在该帐号   ****是否有必要确认？
            ...
            //            NSError *err1;
            //            NSArray<AllUsers*> *result1 = [self.sourceManagedObjectContext executeFetchRequest:requst1 error:&err1];
            ...
            NSError *err2;
            NSArray<User*> *result2 = [self.localManagedObjectContext executeFetchRequest:request2 error:&err2];
            [result2 enumerateObjectsUsingBlock:^(User * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                [self.localManagedObjectContext deleteObject:obj];
            }];
            if (self.localManagedObjectContext.hasChanges) {
                [self.localManagedObjectContext save:nil];
            }
            ...
            NSError *err3;
            NSArray<Contact*> *result3 = [self.localManagedObjectContext executeFetchRequest:request3 error:&err3];
...
-(NSString*)resignUser{
    ...
    //    NSError *err1;
    //    NSArray<User*> *result = [self.localManagedObjectContext executeFetchRequest:request error:&err1];
    //    if (err1) {
...
-(AllUsers*)checkUserInDatabaseWithID:(NSString*)ID Password:(NSString*)password{
    NSEntityDescription *ent = [NSEntityDescription entityForName:@"AllUsers" inManagedObjectContext:self.sourceManagedObjectContext];
...
-(AllUsers*)synchronousDataBaseWithUser:(User*)user{
    ...
    if (sourceUser.wiiError!=nil) {
        ...
        NSArray<AllUsers*> *result = [self.sourceManagedObjectContext executeFetchRequest:request error:&err];
        [result enumerateObjectsUsingBlock:^(AllUsers * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [UserSource copyInfoFromUser:user toAllUsers:obj];
        }];
        if (self.sourceManagedObjectContext.hasChanges) {
            [self.sourceManagedObjectContext save:&err];
        }
        if (err) {
...
-(NSString*)synchronousDataBaseWithUser1:(NSMutableDictionary *)userInfo{
    ...
    NSArray<AllUsers*> *result = [self.sourceManagedObjectContext executeFetchRequest:request error:&err];
    [result enumerateObjectsUsingBlock:^(AllUsers * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [UserSource copyInfoFromLocalUserDic:userInfo toAllUsers:obj];
    }];
    if (self.sourceManagedObjectContext.hasChanges) {
        [self.sourceManagedObjectContext save:nil];
    }
...
-(NSString*)synchronousLocalWithUserInfo:(NSMutableDictionary*)userinfo{
    ...
    request.predicate = [NSPredicate predicateWithFormat:@"wiiID = %@",[userinfo valueForKey:@"wiiID"]];
    NSError *err;
    NSArray<User*> *result = [self.localManagedObjectContext executeFetchRequest:request error:&err];
    [result enumerateObjectsUsingBlock:^(User * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [UserSource copyInfoFromLocalUserDic:userinfo toUser:obj];
    }];
    if (err) {
        resultStr = @"修改本地信息出错";
        return resultStr;
...
-(NSManagedObjectContext*)sourceManagedObjectContext{
    ...
    _sourceManagedObjectContext.persistentStoreCoordinator = [[WiiChatCoreDataStackManager sharedManager] sourcePersistentStoreCoordinator];
...
-(NSManagedObjectContext *)localManagedObjectContext{
    ...
    _localManagedObjectContext.persistentStoreCoordinator = [[WiiChatCoreDataStackManager sharedManager] persistentStoreCoordinator];
...
-(void)copyInfoFromAllUser:(AllUsers*)allusers ToUser:(User*)user{
    ...
    //        Contact *contactInfo = [NSEntityDescription insertNewObjectForEntityForName:@"Contact" inManagedObjectContext:self.localManagedObjectContext];
    //        NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"AllUsers"];
    //        request.predicate = [NSPredicate predicateWithFormat:@"wiiID = %@",contactDic[obj]];
    //        NSArray<AllUsers*> *resultArray = [self.sourceManagedObjectContextexecuteFetchRequest:request error:nil];
    ...
    if (allusers.wiiContact!=nil) {
        for (id obj in allusers.wiiContact) {
            ...
            NSArray<AllUsers*> *resultArray = [self.sourceManagedObjectContext executeFetchRequest:request error:nil];
            [UserSource copyInfoFromDataSource:[resultArray firstObject] toContact:contactInfo];
            ...
            [self.localManagedObjectContext insertObject:user];
